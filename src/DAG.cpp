...
delay_t DAG::getDelayConstraint(const string& node_name){
    auto it = g.delay_map.find(_n.name);                    //Let's pray the compiler would get it
    if(it != g.delay_map.end()){
        return *it;
    }
    else{
        cerr  << "Couldnot retrieve delay from delay constraint map: " << _n.name << endl;
        exit(EXIT_FAILURE);
    }
}


cap_t DAG::getAssignInputTransition(node* in_node, const Library &l){
    //Iterate over all the previous nodes
    //Compute their transition time
    //Return the maximum
    for(string in_node_name: _n.in_nodes){                                      //Over all in_node names
    ...
    auto it = max_element(_n.input_transition_time_list.begin(),_n.input_transition_time_list.end());
    if(it != nput_transition_time_list.end){
        return _n->input_transition_time = *it;                             
    }
    else{
...
cap_t DAG::getAssignOutCapacitance(node* in_node, const Library &l){
    _n->output_cap = 0;
